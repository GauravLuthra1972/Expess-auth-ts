...
var ColorView = Editor.inherit({
    ...
    _renderHtmlRows(updatedOption) {
        var $renderedRows = this._$colorPickerContainer.find(".".concat(COLOR_VIEW_ROW_CLASS));
    ...
    _renderHtmlCellInsideRow: (index, $rowParent, additionalClass) => $("<div>").addClass(COLOR_VIEW_CELL_CLASS).addClass(additionalClass).appendTo($rowParent.find(".".concat(COLOR_VIEW_ROW_CLASS)).eq(index)),
    _renderPalette() {
    ...
    _updateColorFromHsv(hue, saturation, value) {
        ...
        this._currentColor = new Color("hsv(".concat([hue, saturation, value].join(","), ")"));
    ...
    _renderAlphaChannelElements() {
        if (this.option("editAlphaChannel")) {
            this._$colorPickerContainer.find(".".concat(COLOR_VIEW_ROW_CLASS)).eq(1).addClass(COLOR_VIEW_APLHA_CHANNEL_ROW_CLASS);
    ...
    _renderEditorWithLabel(options) {
        var $editor = $("<div>");
        var $label = $("<label>").addClass(options.labelClass).text("".concat(options.labelText, ":")).append($editor);
