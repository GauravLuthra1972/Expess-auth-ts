...
var GOOGLE_URL = "https://maps.googleapis.com/maps/api/js?callback=".concat(GOOGLE_MAP_READY);
...
var GoogleProvider = DynamicProvider.inherit({
    ...
    _loadMapScript() {
        return new Promise(resolve => {
            ...
            ajax.sendRequest({
                url: GOOGLE_URL + (key ? "&key=".concat(key) : ""),
                dataType: "script"
            })
        }).then(() => {
            try {
    ...
    _init() {
        return new Promise(resolve => {
            this._resolveLocation(this._option("center")).then(center => {
                ...
                var listener = google.maps.event.addListener(this._map, "idle", () => {
                    resolve(listener)
                })
            })
        }).then(listener => {
    ...
    _renderRoute(options) {
        return Promise.all(map(options.locations, point => this._resolveLocation(point))).then(locations => new Promise(resolve => {
            ...
            var waypoints = map(locations, location => ({
                location: location,
                stopover: true
            }));
            var request = {
