...
var _m_text_editor = require("../text_box/m_text_editor.base");
...
const Form = _ui2.default.inherit({
    ...
    _applyLabelsWidthByCol($container, index) {
        ...
        (0, _m_label.setLabelWidthByMaxLabelWidth)($container, ".".concat(fieldItemClass).concat(cssExcludeTabbedSelector), labelMarkOptions)
    },
    ...
    _getGroupElementsInColumn($container, columnIndex, colCount) {
        const cssColCountSelector = (0, _type.isDefined)(colCount) ? ".".concat(_constants.GROUP_COL_COUNT_CLASS).concat(colCount) : "";
        const groupSelector = ".".concat(_constants.FORM_FIELD_ITEM_COL_CLASS).concat(columnIndex, " > .").concat(_constants.FIELD_ITEM_CONTENT_CLASS, " > .").concat(_constants.FORM_GROUP_CLASS).concat(cssColCountSelector);
    ...
    _applyLabelsWidthWithGroups($container, colCount, excludeTabbed, labelMarkOptions) {
        ...
        if (alignItemLabelsInAllGroups) {
            ...
            const $groups = this.$element().find(".".concat(_constants.FORM_GROUP_CLASS));
            let i;
    ...
    _alignLabels(layoutManager, inOneColumn) {
        ...
        (0, _visibility_change.triggerResizeEvent)(this.$element().find(".".concat(_m_constants.TOOLBAR_CLASS)))
    },
    _clean() {
    ...
    _itemsOptionChangedHandler(args) {
        ...
        const optionNameWithoutPath = args.fullName.replace("".concat(itemPath, "."), "");
    ...
    _refresh() {
        const editorSelector = ".".concat(_m_text_editor.TEXTEDITOR_CLASS, ".").concat("dx-state-focused", ":not(.").concat(_m_drop_down_editor.DROP_DOWN_EDITOR_CLASS, ") .").concat(_m_text_editor.TEXTEDITOR_INPUT_CLASS);
    ...
    _updateData(data, value, isComplexData) {
        ...
        if ((0, _type.isObject)(_data)) {
            (0, _iterator.each)(_data, (dataField, fieldValue) => {
                that._updateData(isComplexData ? "".concat(data, ".").concat(dataField) : dataField, fieldValue, (0, _type.isObject)(fieldValue))
            })
    ...
    _focusTarget() {
        return this.$element().find(".".concat(_constants.FIELD_ITEM_CONTENT_CLASS, " [tabindex]")).first()
    },
    _visibilityChanged() {
        this._alignLabels(this._rootLayoutManager, this._rootLayoutManager.isSingleColumnMode())
    },
    ...
    getItemID(name) {
        return "dx_".concat(this.option("formID"), "_").concat(name || new _guid.default)
    },
