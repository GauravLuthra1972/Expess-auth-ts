...
const ColorView = _editor.default.inherit({
    ...
    _makeRgba(color) {
        ...
        return "rgba(".concat([color.r, color.g, color.b, color.a].join(", "), ")")
    },
    _renderValue() {
    ...
    _renderHtmlRows(updatedOption) {
        const $renderedRows = this._$colorPickerContainer.find(".".concat(COLOR_VIEW_ROW_CLASS));
    ...
    _renderHtmlCellInsideRow: (index, $rowParent, additionalClass) => (0, _renderer.default)("<div>").addClass(COLOR_VIEW_CELL_CLASS).addClass(additionalClass).appendTo($rowParent.find(".".concat(COLOR_VIEW_ROW_CLASS)).eq(index)),
    _renderPalette() {
    ...
    _updateColorFromHsv(hue, saturation, value) {
        ...
        this._currentColor = new _color.default("hsv(".concat([hue, saturation, value].join(","), ")"));
    ...
    _renderAlphaChannelElements() {
        if (this.option("editAlphaChannel")) {
            this._$colorPickerContainer.find(".".concat(COLOR_VIEW_ROW_CLASS)).eq(1).addClass("dx-colorview-alpha-channel-row");
    ...
    _renderEditorWithLabel(options) {
        ...
        const $label = (0, _renderer.default)("<label>").addClass(options.labelClass).text("".concat(options.labelText, ":")).append($editor);
    ...
    _renderHexInput() {
        ...
        const $hexInput = this._$controlsContainer.find(".".concat("dx-colorview-label-hex")).find(".".concat(TEXT_EDITOR_INPUT));
    ...
    _updateColor(isHex, args) {
        ...
        let newColor;
        if (isHex) {
            newColor = this._validateHex("#".concat(this._hexInput.option("value")))
        } else {
            rgba = this._validateRgb();
            if (this._alphaChannelInput) {
                rgba.push(this._alphaChannelInput.option("value"));
                newColor = "rgba(".concat(rgba.join(", "), ")")
            } else {
                newColor = "rgb(".concat(rgba.join(", "), ")")
            }
        }
