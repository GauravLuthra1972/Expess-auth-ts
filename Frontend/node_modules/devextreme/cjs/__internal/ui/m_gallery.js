...
const GALLERY_WRAPPER_CLASS = "".concat("dx-gallery", "-wrapper");
const GALLERY_LOOP_CLASS = "dx-gallery-loop";
const GALLERY_ITEM_CONTAINER_CLASS = "".concat("dx-gallery", "-container");
const GALLERY_ACTIVE_CLASS = "".concat("dx-gallery", "-active");
const GALLERY_ITEM_CLASS = "".concat("dx-gallery", "-item");
const GALLERY_INVISIBLE_ITEM_CLASS = "".concat("dx-gallery", "-item-invisible");
const GALLERY_LOOP_ITEM_CLASS = "".concat(GALLERY_ITEM_CLASS, "-loop");
const GALLERY_ITEM_SELECTOR = ".".concat(GALLERY_ITEM_CLASS);
const GALLERY_ITEM_SELECTED_CLASS = "".concat(GALLERY_ITEM_CLASS, "-selected");
const GALLERY_INDICATOR_CLASS = "".concat("dx-gallery", "-indicator");
const GALLERY_INDICATOR_ITEM_CLASS = "".concat(GALLERY_INDICATOR_CLASS, "-item");
const GALLERY_INDICATOR_ITEM_SELECTOR = ".".concat(GALLERY_INDICATOR_ITEM_CLASS);
const GALLERY_INDICATOR_ITEM_SELECTED_CLASS = "".concat(GALLERY_INDICATOR_ITEM_CLASS, "-selected");
...
const GalleryNavButton = _ui.default.inherit({
    ...
    _render() {
        ...
        $element.addClass("".concat("dx-gallery", "-nav-button-").concat(this.option("direction")));
...
const Gallery = _uiCollection_widget.default.inherit({
    ...
    _getRealItems() {
        const selector = ".".concat(GALLERY_ITEM_CLASS, ":not(.").concat(GALLERY_LOOP_ITEM_CLASS, ")");
    ...
    _renderItemSizes(startIndex) {
        ...
        $items.each(index => {
            (0, _size.setOuterWidth)((0, _renderer.default)($items[index]), "".concat(100 * itemWidth, "%"))
        })
    },
    _renderItemPositions() {
        ...
        const loopItemsCount = this.$element().find(".".concat(GALLERY_LOOP_ITEM_CLASS)).length;
        ...
        this._itemElements().each((function(index) {
            ...
            if (!isLoopItem && 0 !== realIndex) {
                if (isGapBetweenImages) {
                    (0, _renderer.default)(this).css("margin".concat(side), "".concat(100 * freeSpace, "%"))
                }
                return
            }
            const itemPosition = itemWidth * (realIndex + offsetRatio) + freeSpace * (realIndex + 1 - offsetRatio);
            const property = isLoopItem ? side.toLowerCase() : "margin".concat(side);
            (0, _renderer.default)(this).css(property, "".concat(100 * itemPosition, "%"))
        }));
    ...
    _toggleIndicatorInteraction(clickEnabled) {
        var _a;
        const $indicatorItems = (null === (_a = this._$indicator) || void 0 === _a ? void 0 : _a.find(GALLERY_INDICATOR_ITEM_SELECTOR)) || [];
    ...
    _renderItemVisibility() {
        ...
        this._itemElements().each((index, item) => {
            if (selectedIndex !== index) {
                (0, _renderer.default)(item).find(".dx-item-content").addClass(GALLERY_INVISIBLE_ITEM_CLASS)
            }
        })
    },
    _releaseInvisibleItems() {
        this._itemElements().find(".dx-item-content").removeClass(GALLERY_INVISIBLE_ITEM_CLASS)
    },
    ...
    _setFocusOnSelect() {
        this._userInteraction = true;
        const selectedItem = this._getRealItems().filter(".".concat(GALLERY_ITEM_SELECTED_CLASS));
