...
const ProgressBar = _track_bar.default.inherit({
    _getDefaultOptions() {
        return (0, _extend.extend)(this.callBase(), {
            value: 0,
            statusFormat: ratio => "Progress: ".concat(Math.round(100 * ratio), "%"),
            showStatus: true,
            onComplete: null,
    ...
    _renderIndeterminateState() {
        ...
        for (let i = 0; i < segments; i++) {
            (0, _renderer.default)("<div>").addClass(PROGRESSBAR_INDETERMINATE_SEGMENT).addClass("".concat(PROGRESSBAR_INDETERMINATE_SEGMENT, "-").concat(i + 1)).appendTo(this._$segmentContainer)
        }
    ...
    _togglePositionClass() {
        ...
        let positionClass = "dx-position-".concat(splitPosition[0]);
        if (splitPosition[1]) {
            positionClass += "-".concat(splitPosition[1])
        }
