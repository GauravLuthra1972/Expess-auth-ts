...
const TreeViewBase = _ui.default.inherit({
    ...
    _renderNodeContainer($parent) {
        ...
        if ($parent && $parent.length) {
            const itemData = this._getItemData($parent.children(".".concat(ITEM_CLASS)));
    ...
    _createDOMElement($nodeContainer, node) {
        ...
        const hasChildNodes = !!(null === (_b = null === (_a = null === node || void 0 === node ? void 0 : node.internalFields) || void 0 === _a ? void 0 : _a.childrenKeys) || void 0 === _b ? void 0 : _b.length);
        if (hasChildNodes) {
    ...
    _getNodeByElement(itemElement) {
        const $node = (0, _renderer.default)(itemElement).closest(".".concat(NODE_CLASS));
    ...
    _normalizeIconState($node, hasNewItems) {
        ...
        if ($loadIndicator.length) {
            null === (_a = _load_indicator.default.getInstance($loadIndicator)) || void 0 === _a ? void 0 : _a.option("visible", false)
        }
        const $treeViewItem = this._getItem($node);
    ...
    _toggleNodeDisabledState(node, state) {
        const $node = this._getNodeElement(node);
        const $item = $node.find(".".concat(ITEM_CLASS)).eq(0);
    ...
    _updateParentsState(node, $node) {
        ...
        const $parentNode = (0, _renderer.default)($node.parents(".".concat(NODE_CLASS))[0]);
        if (this._showCheckboxes()) {
    ...
    _itemEventHandlerImpl(initiator, action, actionArgs) {
        const $itemElement = (0, _renderer.default)(initiator).closest(".".concat(NODE_CLASS)).children(".".concat(ITEM_CLASS));
    ...
    _focusInHandler(e) {
        ...
        this.option("focusedElement", (0, _element.getPublicElement)($activeItem.closest(".".concat(NODE_CLASS))))
    },
    _itemPointerDownHandler(e) {
        if (!this.option("focusStateEnabled")) {
            return
        }
        const $target = (0, _renderer.default)(e.target).closest(".".concat(NODE_CLASS, ", .").concat(SELECT_ALL_ITEM_CLASS));
        if (!$target.length) {
            return
        }
    ...
    _findNonDisabledNodes: $nodes => $nodes.not((function() {
        return (0, _renderer.default)(this).children(".".concat(ITEM_CLASS)).hasClass("dx-state-disabled")
    })),
